...
def get_oa_urls():
    try:
        ...
        pmid_url_dict = {pmid[0].replace('https://pubmed.ncbi.nlm.nih.gov/', ''): oa_url for pmid, oa_url in zip(pmid_values, oa_urls)}

        pmid_url_df = pd.DataFrame(pmid_url_dict.items(), columns=['PMID', 'oa_url'])
