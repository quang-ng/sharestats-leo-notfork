...
def filter_csv_files(csv_files_path, pmid_df, download_path):
    ...
    for csv_file in csv_files_path.rglob('*.csv'):
        ...
        relative_path_str = str(relative_path).split('/')[-1].replace('.filelist.csv', '.tar.gz')

        # Exclude rows where "Article File" matches existing files in download_path
        filtered_data = csv_data[
            (csv_data['PMID'].isin(pmid_df['PMID']))
        ]
    ...
    filter_condition = (~final_result['Article File'].apply(lambda x: any(existing_file in x for existing_file in existing_files)))
    ...
    if response.status_code == 200:
        ...
        for tar_gz_link in tar_gz_links:
            ...
            if tar_gz_filename in unique_filenames:
                ...
                print(tar_gz_url)
                tar_gz_content = requests.get(tar_gz_url, headers=headers).content
                with open(download_path / tar_gz_filename, 'wb') as tar_gz_file:
                    tar_gz_file.write(tar_gz_content)
                    # Uncompress the downloaded .tar.gz file
                    with tarfile.open(fileobj=BytesIO(tar_gz_content), mode='r:gz') as tar:
                        ...
                        files_to_keep_names = [f for f in files_in_tar if f in files_to_keep['Article File'].tolist()]
                        print(len(files_to_keep_names), files_to_keep_names[:5])
...
for web_directory_url in web_directory_urls:
    extract_and_keep_files(web_directory_url, download_path, files_to_keep['Relative Path'].unique())
