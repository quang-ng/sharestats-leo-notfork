...
if __name__ == "__main__":
    ...
    pdf_pmids = [int(pdf_file.strip('.pdf')) for pdf_file in os.listdir(download_folder) if pdf_file.endswith('.pdf')]
    ...
    url_chunks = [list(zip(urls[i:i+chunk_size], pmids[i:i+chunk_size])) for i in range(0, len(urls), chunk_size)]
    ...
    for chunk in url_chunks:
        result = pool.apply_async(process_urls_chunk, (chunk, download_folder, ERROR_LOG_FILE_PATH))
